...
##-------------------------------------------------------------------------
# draw_tree.draw_path(dt, dt.classes_, X.columns, 'path')
##-------------------------------------------------------------------------
def draw(tree, class_names, feature_names, output, impurity=False):
...
##-------------------------------------------------------------------------
# p = dt.decision_path(Xtest)
# draw_tree.draw_path(dt, dt.classes_, X.columns, p.indices, 'path')
##-------------------------------------------------------------------------
def draw_path(tree, class_names, feature_names, path, output):
	with open(output+".dot", 'w') as f:
		dot_data = export_graphviz(
			tree,
			out_file = f,
...
def export_graphviz(decision_tree, out_file=SENTINEL, max_depth=None,
    ...
    def recurse(tree, node_id, criterion, parent=None, depth=0):
        ...
        if max_depth is None or depth <= max_depth:
            ...
            # Mark nodes (on a decision path)
            if node_id in marked_nodes:
	            out_file.write(', color=red')
